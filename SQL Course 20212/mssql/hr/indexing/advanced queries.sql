...
-- 3
go
create index jh_start_end_date on job_history(START_DATE, END_DATE);
go
...
-- 4
go 
create index jobs_title on jobs(job_title);
go
...
-- 1
go
create index employees_hiredate on employees(hire_date);
go

select first_name, last_name from employees 
with(index(employees_hiredate))
